...
export default {
  ...
  methods:{
    ...
    async loadArticle(){
      ...
      const res = await Article.findOne(id)
      
      if(res.success){

        this.mode = 'update'

        const { description, title, content, photo, _id } = res.data.result

        this.article = { description, title, content, photo, _id }
  ...
  mounted(){
    ...
    if( this.mode !== 'save'  ) this.loadArticle()
